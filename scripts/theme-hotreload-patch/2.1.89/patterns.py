"""
Patch patterns for Claude Code 2.1.89.

Architecture:
  The file watcher (GR4) polls ~/.claude.json every 1s and updates the config
  cache (Od). The ThemeProvider reads theme into React state once at mount.
  ...
  ThemeProvider: pG_
    ...
    Hook: W6H() = useContext(InternalStdinContext)
    No-op effect: useEffect(()=>{}, [w,f])

  File watcher: GR4
    After re-reading config: Od = {config: ..., mtime: ...}, M6H = {mtime: ..., size: ...}
...
PATCHES = [
    (
        ...
        b'Od={config:nk6({...Tn(),...K}),mtime:_.mtimeMs},M6H={mtime:_.mtimeMs,size:_.size}}).catch',
        b'Od={config:nk6({...Tn(),...K}),mtime:_.mtimeMs},M6H={mtime:_.mtimeMs,size:_.size},globalThis._t&&K.theme&&globalThis._t(K.theme)}).catch',
    ),
    (
        ...
        b',{internal_querier:f}=W6H();UG.useEffect(()=>{},[w,f]);let Y=w==="auto"?z:w,j=UG.useMemo(()=>({themeSetting:K,setThemeSetting:(D)=>{if($(D),T(null),D==="auto")A(rUH());q?.(D)},setPreviewTheme:(D)=>{if(T(D),D==="auto")A(rUH())},savePreview:()=>{if(O!==null)$(O),T(null),q?.(O)},cancelPreview:()=>{if(O!==null)T(null)}',
        b',f=W6H();UG.useEffect(()=>{globalThis._t=$},[]);let Y=w=="auto"?z:w,j=UG.useMemo(()=>({themeSetting:K,setThemeSetting:(D)=>{if($(D),T(null),D=="auto")A(rUH());q?.(D)},setPreviewTheme:(D)=>{if(T(D),D=="auto")A(rUH())},savePreview:()=>{if(O!=null)$(O),T(null),q?.(O)},cancelPreview:()=>{if(O!=null)T(null)}',
    ),
    (
        "Yq hook narrowing",
        # !== to != in useTheme hook cache check (compares React elements, safe)
        b'if(H[0]!==_||H[1]!==q)K=[_,q],H[0]=_,H[1]=q,H[2]=K;else K=H[2];return K}function jGH',
        b'if(H[0]!=_||H[1]!=q)K=[_,q],H[0]=_,H[1]=q,H[2]=K;else K=H[2];return K}function jGH',
    ),
    (
        "gG_ hook narrowing",
